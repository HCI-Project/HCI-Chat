...
public class SocketServer implements Runnable {
    ...
    public SocketServer(ChatFrame frame){
	...
	try{  
	    ...
	    ui.chatText.append("Server started. IP : " + InetAddress.getLocalHost() + ", Port : " + server.getLocalPort());
	    start(); 
        }
	catch(IOException ioe){  
            ui.chatText.append("Can not bind to port : " + port + "\nRetrying"); 
            ui.RetryStart(0);
	}
    }
    
    public SocketServer(ChatFrame frame, int Port){
  ...
  while (thread != null){  
   ...
   /* public void run(){  
    ...
    public void Announce(String type, String sender, String content){
        ...
        for(int i = 0; i < clientCount; i++){
            if(msg.sender != clients[i].username){
            clients[i].send(msg);
            }
        }
